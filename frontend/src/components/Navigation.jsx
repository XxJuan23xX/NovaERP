...
export default function Navigation() {
  ...
  const navItems = [
    { name: 'Dashboard', path: user?.role === 'admin' ? '/dashboard' : '/operaciones' },
    { name: 'Productos', path: '/inventario/productos' },
  ]

  // Solo admin tiene acceso al Kardex
  if (user?.role === 'admin') {
    navItems.push({ name: 'Historial & Ajustes (Kardex)', path: '/inventario/kardex' })
  }
  ...
  return (
    <header className="sticky top-0 z-40 bg-white border-b border-slate-200 px-6 py-3.5 flex justify-between items-center shadow-sm">
      <div className="flex items-center gap-8">
        {/* Brand Logo */}
        <Link to={user?.role === 'admin' ? '/dashboard' : '/operaciones'} className="flex items-center gap-2.5">
