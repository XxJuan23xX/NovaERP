...
export default function CierreCajaPage() {
  ...
  const handleConfirmarCierre = async (e) => {
    e.preventDefault()
    if (efectivoFisico === '') {
      setErrorMensaje('Por favor, ingresa el efectivo físico actual de la caja.')
      return
    }
    ...
    try {
      ...
      const errDetail = err.response?.data?.message || 'Error en el servidor al confirmar el cierre.'
  ...
  const handleExportar = () => {
    ...
    csvContent += 'Empleado;Tickets;Efectivo (Monto);Efectivo (Ventas);Tarjeta (Monto);Tarjeta (Ventas);Cancelaciones (Monto);Total Neto\n'
      ...
      <main className="flex-1 min-w-0 flex flex-col p-8 overflow-y-auto print:p-0">
        ...
        {errorMensaje && (
          <div className="bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-2xl mb-6 flex items-start gap-3 shadow-sm print:hidden">
            ...
            <button onClick={() => setErrorMensaje('')} className="ml-auto text-red-650 hover:text-red-800 font-bold text-xs select-none cursor-pointer">Ocultar</button>
          </div>
        )}
        ...
        {successMensaje && (
          <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 px-4 py-3 rounded-2xl mb-6 flex items-start gap-3 shadow-sm print:hidden animate-in fade-in zoom-in-95 duration-200">
            <svg className="h-5 w-5 mt-0.5 flex-shrink-0 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
            </svg>
            <span className="font-extrabold text-sm">{successMensaje}</span>
            <button onClick={() => setSuccessMensaje('')} className="ml-auto text-emerald-650 hover:text-emerald-850 font-bold text-xs select-none cursor-pointer">Ocultar</button>
          </div>
        )}
          ...
          <div className="space-y-8 print:space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 print:grid-cols-4 print:gap-3">
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center gap-4 hover:shadow transition-all print:border print:shadow-none">
                ...
                <div>
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-500">Total Vendido</span>
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center gap-4 hover:shadow transition-all print:border print:shadow-none">
                ...
                <div>
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-500">Efectivo Recibido</span>
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center gap-4 hover:shadow transition-all print:border print:shadow-none">
                ...
                <div>
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-500">Ventas Tarjeta</span>
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center gap-4 hover:shadow transition-all print:border print:shadow-none">
                ...
                <div>
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-500">Cancelaciones</span>
                        ...
                        cajaData.empleados?.map((emp) => (
                          <tr key={emp.user_id} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="py-3.5 pr-2 flex items-center gap-3">
                              ...
                              <div>
                                <div className="text-[13px] font-bold text-slate-900">{emp.nombre}</div>
                                <span className="text-[10px] text-slate-500 font-semibold">Cajero ID: #{emp.user_id}</span>
                              </div>
                            </td>
                            {/* Tickets */}
                            ...
                            <td className="py-3.5 text-right font-medium">
                              <span className="block text-slate-900 font-bold">{formatPrice(emp.efectivo_monto)}</span>
                              <span className="text-[10px] text-slate-500 font-semibold">{emp.efectivo_ventas} {emp.efectivo_ventas === 1 ? 'vta.' : 'vtas.'}</span>
                            </td>
                            {/* Tarjeta */}
                            <td className="py-3.5 text-right font-medium">
                              <span className="block text-slate-900 font-bold">{formatPrice(emp.tarjeta_monto)}</span>
                              <span className="text-[10px] text-slate-500 font-semibold">{emp.tarjeta_ventas} {emp.tarjeta_ventas === 1 ? 'vta.' : 'vtas.'}</span>
                            </td>
                            {/* Cancelaciones */}
                            <td className="py-3.5 text-right text-red-600 font-medium">
                              <span className="block font-bold">-{formatPrice(emp.cancelaciones_monto)}</span>
                              <span className="text-[10px] font-semibold">{emp.cancelaciones_ventas} {emp.cancelaciones_ventas === 1 ? 'vta.' : 'vtas.'}</span>
                            </td>
                            {/* Total Neto */}
              ...
              <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col space-y-6 print:border print:shadow-none">
                ...
                <div className="space-y-3.5 text-xs font-semibold text-slate-650 bg-slate-50 border border-slate-200/60 p-4 rounded-2xl shadow-inner select-none print:bg-white">
                  <div className="flex justify-between">
                    ...
                    <span className="text-slate-900 font-black">{cajaData.sesion?.caja_nombre}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Almacén sucursal</span>
                    <span className="text-slate-900 font-bold">{cajaData.sesion?.almacen_nombre}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Cajero de apertura</span>
                    <span className="text-slate-900 font-bold">{cajaData.sesion?.cajero_nombre}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Fecha Apertura</span>
                    <span className="text-slate-800 font-medium">
                      {cajaData.sesion?.fecha_apertura ? new Date(cajaData.sesion.fecha_apertura).toLocaleString('es-MX', { dateStyle: 'short', timeStyle: 'short' }) : '-'}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-xl text-center print:bg-white">
                      <span className="text-[10px] font-black uppercase text-slate-500">Stock Actual</span>
                      <div className="text-slate-950 font-black text-lg mt-1">{cajaData.inventario.stock_total} uds.</div>
                    </div>
                    <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-xl text-center print:bg-white">
                      <span className="text-[10px] font-black uppercase text-slate-500">Valor Ventas</span>
                      <div className="text-indigo-650 font-black text-lg mt-1">{formatPrice(cajaData.inventario.valor_venta_total)}</div>
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-white border border-slate-250 rounded-3xl p-8 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6 print:border print:shadow-none print:p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 flex-1 w-full max-w-2xl select-none print:gap-3">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-slate-500 font-medium">Establecido al abrir caja</span>
                </div>
                ...
                <div>
                  ...
                  <span className="text-[10px] text-slate-500 font-medium">Registrado por el sistema</span>
                </div>
                ...
                <div>
                  ...
                  <span className="text-[10px] text-indigo-500 font-semibold">Fondo + Efectivo de ventas</span>
                </div>

              </div>

              {/* Formulario de Arqueo y Cierre */}
              <form onSubmit={handleConfirmarCierre} className="flex flex-col sm:flex-row items-end sm:items-center gap-4 w-full md:w-auto border-t md:border-t-0 md:border-l border-slate-200 pt-6 md:pt-0 md:pl-6 print:border-0 print:pt-0 print:pl-0">
                
                {/* Entrada de Efectivo Real */}
                <div className="w-full sm:w-auto">
                  <label htmlFor="efectivo_real" className="text-[10px] font-black uppercase tracking-wider text-slate-700 block mb-1.5 select-none">
                  ...
                  <div className="relative rounded-xl shadow-sm">
                    ...
                    <input
                      ...
                      className={`w-full sm:w-44 bg-white border border-slate-350 text-slate-900 placeholder-slate-400 font-bold rounded-xl pl-8 pr-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-sm ${
                        esCerrada ? 'bg-slate-50 text-slate-500 cursor-not-allowed border-slate-200' : ''
                      }`}
                      required
                    />
                  </div>
                ...
                {efectivoFisico !== '' && (
                  <div className="text-right sm:text-left select-none">
                    ...
                    >
                      {descuadre === 0 ? 'Sin descuadre (Exacto)' : descuadre > 0 ? `Sobrante: +${formatPrice(descuadre)}` : `Faltante: ${formatPrice(descuadre)}`}
                    </span>
                  </div>
                )}
